...
def map_columns(df, db_columns, type, mapping_dict):
    ...
    # Exclude columns that shouldn't be mapped manually
    excluded_columns = ["id", "staging_status", "tags", "date_uploaded", "date_modified"]
    
    # Include linking fields for demographic and contact types
    ...
    # Normalize file columns to match database columns (case-insensitive)
    ...
    # Create a DataFrame for manual adjustments
    ...
    # Display editable mapping interface in Streamlit
    mapping_df = st.data_editor(
        ...
        column_config={
            ...
            "File Column Mapping": st.column_config.SelectboxColumn(
                "File Column Mapping",
                options=["None"] + list(df.columns),  # Provide dropdown options from file columns
            ),
        },
        hide_index=True,
...
def determine_staging_status(uploaded_df, table_name, table_model):
    ...
    if table_name == "scrubbed":
        ...
        primary_keys = ["scrubbed_id"]  # Foreign key reference
    else:
    ...
    for record in existing_records:
        ...
        existing_dict[key_tuple] = record  # entire row as a dict

    # 4) Compare for each row in uploaded_df
    staging_status = []
    ...
    for _, row in uploaded_df.iterrows():
        # Make the PK tuple from the row
        ...
        if pk_tuple not in existing_dict:
            ...
            # Found in DB => check if any non-PK columns differ
            db_row = existing_dict[pk_tuple]
            is_updated = False
            for col in non_pk_cols:
                # compare new vs old
                new_val = row[col]
                old_val = db_row.get(col)  # from the DB row dict
                if new_val != old_val:
...
def bulk_upsert_to_db(df, table_model, table_name, primary_keys):
    """
    Optimized bulk insert/update using `bulk_insert_mappings()` and `bulk_update_mappings()`.
    Ensures `date_uploaded` and `date_modified` are included.

    Args:
        df (pd.DataFrame): The DataFrame to insert/update.
        table_model (SQLAlchemy model): The ORM model to target.
        table_name (str): Table name.
        primary_keys (list): The list of primary key columns.
    ...
    try:
        if df.empty:
            st.warning(f"No data to upsert into '{table_name}'.")
            return
        df = preprocess_all_columns(df, table_model)
        df = clean_dataframe_for_sql(df)
        
        current_date = datetime.now().date()  # or .now() if you want a datetime
        df["date_uploaded"] = current_date
        df["date_modified"] = current_date

        data_dicts = df.to_dict(orient="records")  

        query = f"SELECT {', '.join(primary_keys)} FROM {table_name}"
        with engine.connect() as connection:
            existing_keys = set(tuple(row) for row in pd.read_sql(query, connection)[primary_keys].to_numpy())

        new_records = []
        update_records = []
        
        for record in data_dicts:
            key_tuple = tuple(record[key] for key in primary_keys)
        ...
        with Session() as session: 
            if new_records:
                ...
                session.commit()
                st.success(f"Inserted {len(new_records)} new records into '{table_name}'.")

            # Perform batch update for existing records
            if update_records:
                session.bulk_update_mappings(table_model, update_records)
                session.commit()
                st.success(f"Updated {len(update_records)} existing records in '{table_name}'.")

    except Exception as e:
        st.error(f"Error in bulk_upsert_to_db(): {e}")



def upload_data_to_db(df, table_model, table_name, column_mapping):
    """
    Uploads a DataFrame to the database using PostgreSQL COPY for fast bulk inserts.
    Uses a temporary table, COPY for inserts, and a fast batch UPDATE for existing records.
    Ensures staging ('New', 'Updated', 'Unchanged') before uploading.
    Prevents duplicates by inserting only the 'New' rows and updating only 'Updated' rows.
    """
    try:
        if df.empty:
        ...
        # A. Identify primary keys dynamically
        if table_name == "scrubbed":
            primary_keys = [
                "owner_first_name", "owner_last_name", "mail_zip",
            ...
            if "tags" not in df.columns:
                df["tags"] = None  # Ensure column exists for scrubbed
        elif table_name in ["demographic_append", "contact_append"]:
        ...
        # B. Rename CSV columns -> DB columns (based on column_mapping)
        # selected_columns = {db_col: file_col}, so we invert for rename
        selected_columns = {
            file_col: db_col 
        ...
        # If 'tags' is in the DF but not mapped yet, add it
        if "tags" in df.columns and "tags" not in selected_cols and table_name == "scrubbed":
            selected_cols.append("tags")

        # Keep only the columns we intend to load
        df = df[selected_cols].rename(columns=selected_columns)

        # For demographic/contact, map scrubbed_id from the 'scrubbed' table
        if table_name in ["demographic_append", "contact_append"]:
            df = map_scrubbed_id(df)
            df = df[df["scrubbed_id"].notnull()].copy()
        
        if table_name in ["scrubbed"]:
        ...
        # 2) Build a dictionary: key -> date_uploaded
        dates_dict = {}
        ...
        # 3) Fill in date_uploaded for Updated/Unchanged
        ...
        # For updated rows
        df.loc[df["staging_status"] == "Updated", "date_modified"] = current_date

        # D. Preprocess columns to match SQLAlchemy types
        df = preprocess_all_columns(df, table_model)
        df = clean_dataframe_for_sql(df)
        df = df.drop(columns=["id"], errors="ignore")

        # E. Re-order columns to match desired structure
        if table_name == "scrubbed":
            df = df[SCRUBBED_DESIRED_ORDER]
        elif table_name == "demographic_append":
            df = df[DEMOGRAPHIC_DESIRED_ORDER]
        elif table_name == "contact_append":
            df = df[CONTACT_DESIRED_ORDER]

        # F. Fix date_transfer if it exists
        if "date_transfer" in df.columns:
            df["date_transfer"] = pd.to_datetime(df["date_transfer"], errors="coerce")
            df["date_transfer"] = df["date_transfer"].dt.date

        # G. Create a temporary staging table and COPY
        ...
        with engine.begin() as connection:
            ...
            # 1) Fetch the actual columns in the staging table
            ...
            print("Staging table columns (in DB order):", staging_cols)

            # 2) Compare to your DataFrame columns
            ...
            # I. Update only 'Updated' rows
            # Build the list of columns to update, skipping primary keys and 'tags' logic if needed
            update_assignments = []
            skip_cols = set(primary_keys + ["staging_status"])
            for col in columns_list:
                if col in skip_cols:
                    continue
                if col == "tags" and table_name == "scrubbed":
                    # If you want to keep existing tags if not null:
            ...
            if table_name == "scrubbed":
                ...
                # no tags logic for demographic/contact
                update_sql = f"""
                    UPDATE {table_name}
                    SET {', '.join(update_assignments)}
                    FROM {temp_table}
        ...
        st.success(f"Successfully upserted {len(df)} records into '{table_name}' using \copy and batch UPDATE.")

    except Exception as e:
        st.error(f"Error in upload_data_to_db(): {e}")
        ...
        try:
            ...
            if include_scrubbed:
                ...
                if st.button("Upload Scrubbed Data"):
                    upload_data_to_db(df, Scrubbed, "scrubbed", scrubbed_mapping)
            ...
            if include_demographic:
                ...
                if st.button("Upload Demographic Append Data"):
                    upload_data_to_db(df, Demographic_Append, "demographic_append", demographic_mapping)
            ...
            if include_contact:
                ...
                if st.button("Upload Contact Append Data"):
                    upload_data_to_db(df, Contact_Append, "contact_append", contact_mapping)
